...
async def _collect_finance(date_from: Optional[str], date_to: Optional[str]):
    q = {}
    ...
    # KPI somme
    pipeline_sum = []
    if q: pipeline_sum.append({"$match": q})
    pipeline_sum += [
        {"$group": {"_id": None,
                    "sum_total": {"$sum": "$total"},
                    "sum_paid":  {"$sum": {"$cond": [{"$eq": ["$paid", True]}, "$total", 0]}}}}
    ]
    ...
    async for d in cur:
        rows.append({
            ...
            "customer": (d.get("customer_snapshot", {}) or {}).get("company_name")
                        or f"{(d.get('customer_snapshot', {}) or {}).get('first_name','')} {(d.get('customer_snapshot', {}) or {}).get('last_name','')}".strip(),
            "status": d.get("status"),
...
async def finance_json(
    ...
    return await _collect_finance(date_from, date_to)
...
async def finance_export_xlsx(
    ...
    """
    Esporta un report Excel con:
    - KPI (totale, pagato, outstanding)
    - Invoices (tabellare)
    """
    try:
        import openpyxl
        ...
        # Se mancano le dipendenze, alza un 500 “gentile”
        from fastapi import HTTPException
        raise HTTPException(500, detail="openpyxl non installato. Esegui: pip install openpyxl")

    data = await _collect_finance(date_from, date_to)
    ...
    headers = ["Numero", "Data", "Scadenza", "Cliente", "Stato", "Pagata", "Data Pag.", "Imponibile", "IVA", "Totale"]
    ws2.append(headers)
    for c in range(1, len(headers)+1):
        ws2.cell(row=1, column=c).font = Font(bold=True)
        ws2.cell(row=1, column=c).fill = PatternFill("solid", fgColor="EEEEEE")

    for r in data["rows"]:
        ws2.append([
            r["number"], r["issue_date"], r["due_date"], r["customer"], r["status"],
            "Sì" if r["paid"] else "No", r["paid_at"], r["subtotal"], r["vat_total"], r["total"]
        ])
